...
func AutoUpdate(repair bool, output io.Writer) {
	...
	if utils.IsFileOrDirectoryExists("update_info.json") {
		err = localUpdateInfo.LoadFromJSON(utils.ReadStringFromFile("update_info.json"))
		if err != nil {
	...
	if localUpdateInfo.GameVersion < newUpdateInfo.GameVersion || repair { //版本旧了的话就更新
		...
		fileListJSON := utils.ReadStringFromURL(resourceURL + "file_list.json")
		...
		for k := range *surp {
			...
			if utils.IsFileOrDirectoryExists(fn) {
				os.Remove(fn)
			}
			os.Rename(k, fn)
			//fmt.Println("已移动至：" + fn)
		...
		for k, v := range *lack {
			go func(path, hash string) {
				...
				conflictSuffix := ""
				for !ok {
					if !utils.IsFileOrDirectoryExists(filepath.Join("download", hash+".zip"+conflictSuffix)) {
...
func Pack(init bool) {
	...
	oldFileList := GetFileListFromJSON(utils.ReadStringFromURL(resourceURL + "file_list.json")) //服务器上的文件列表
